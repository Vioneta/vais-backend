...
def update():
    ...
    with open(f"{os.getcwd()}/config/.storage/vais.repositories", encoding="utf-8") as storage_file:
        storage = json.load(storage_file)

    with open(
        f"{os.getcwd()}/custom_components/vais/utils/default.repositories", encoding="utf-8"
    ) as old_file:
    ...
    with open(
        f"{os.getcwd()}/custom_components/vais/utils/default.repositories",
        mode="w",
        encoding="utf-8",
    ) as to_store_file:
