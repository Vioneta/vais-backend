...
from .utils.configuration_schema import RELEASE_LIMIT, vais_config_option_schema
from .utils.logger import get_vais_logger


class VaisFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """Config flow for VAIS."""
        ...
        if not self.activation:
            ...
            if not self.device:
                self.device = GitHubDeviceAPI(
                    ...
                    **{"client_name": f"VAIS/{integration.version}"},
                )
            async_call_later(self.hass, 1, _wait_for_activation)
            try:
    ...
    def async_get_options_flow(config_entry):
        return VaisOptionsFlowHandler(config_entry)


class VaisOptionsFlowHandler(config_entries.OptionsFlow):
    """VAIS config flow options handler."""

    def __init__(self, config_entry):
        """Initialize VAIS options flow."""
    ...
    async def async_step_user(self, user_input=None):
        """Handle a flow initialized by the user."""
        vais: VaisBase = self.hass.data.get(DOMAIN)
        ...
        if vais is None or vais.configuration is None:
            return self.async_abort(reason="not_setup")

        if vais.configuration.config_type == ConfigurationType.YAML:
            schema = {vol.Optional("not_in_use", default=""): str}
        else:
            schema = vais_config_option_schema(self.config_entry.options)
