...
from .entity import VaisRepositoryEntity
...
async def async_setup_entry(hass, _config_entry, async_add_devices):
    ...
    async_add_devices(
        VaisRepositoryUpdateEntity(vais=vais, repository=repository)
        for repository in vais.repositories.list_downloaded
    )


class VaisRepositoryUpdateEntity(VaisRepositoryEntity, UpdateEntity):
    """Update entities for repositories downloaded with VAIS."""
    ...
    def entity_picture(self) -> str | None:
        ...
        if (
            self.repository.data.category != VaisCategory.INTEGRATION
