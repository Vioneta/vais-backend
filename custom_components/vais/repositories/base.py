...
from ..enums import ConfigurationType, VaisDispatchEvent, RepositoryFile
from ..exceptions import (
    VaisException,
    VaisNotModifiedException,
    ...
    def update_data(self, data: dict):
        ...
        for key in data:
            ...
            if key == "pushed_at":
                ...
                setattr(
                    self, key, [topic for topic in data[key] if topic not in TOPIC_FILTER])
...
class VaisManifest:
    ...
    country: List[str] = []
    filename: str = None
    vais: str = None  # Minimum VAIS version
    ...
    def from_dict(manifest: dict):
        """Set attributes from dicts."""
        if manifest is None:
            raise VaisException("Missing manifest data")

        manifest_data = VaisManifest()
...
class VaisRepository:
    """VaisRepository."""

    def __init__(self, vais: VaisBase) -> None:
        """Set up VaisRepository."""
        self.vais = vais
        ...
        self.repository_manifest = VaisManifest.from_dict({})
    ...
    def ignored_by_country_configuration(self) -> bool:
        ...
        if self.data.installed:
            return False
        configuration = self.vais.configuration.country.lower()
        if configuration == "all":
            return False

        manifest = [entry.lower()
                    for entry in self.repository_manifest.country or []]
        if not manifest:
            return False
        ...
        if self.repository_manifest.homeassistant is not None:
            if self.data.releases:
                if not version_left_higher_or_equal_then_right(
                    self.vais.core.ha_version.string,
    ...
    async def common_validate(self, ignore_issues: bool = False) -> None:
        ...
        # Get the content of vais.json
        if RepositoryFile.VAIS_JSON in [x.filename for x in self.tree]:
            if manifest := await self.async_get_vais_json():
                self.repository_manifest = VaisManifest.from_dict(manifest)
    ...
    async def common_registration(self) -> None:
        ...
        if self.repository_object is None:
            ...
            except VaisNotModifiedException:
                self.logger.debug(
    ...
    async def common_update(self, ignore_issues=False, force=False) -> bool:
        ...
        except VaisRepositoryExistException:
            self.data.full_name = self.vais.common.renamed_repositories[self.data.full_name]
            await self.common_update_data(ignore_issues=ignore_issues, force=force)
        ...
        try:
            ...
            if not contents:
                validate.errors.append(
                    f"No assets found for release '{self.ref}'")
                return

            download_queue = QueueManager(hass=self.vais.hass)
    ...
    async def async_download_zip_file(self, content, validate) -> None:
        """Download ZIP archive from repository release."""
        try:
            filecontent = await self.vais.async_download_file(content.browser_download_url)

            if filecontent is None:
                validate.errors.append(f"[{content.name}] was not downloaded")
                return

            temp_dir = await self.vais.hass.async_add_executor_job(tempfile.mkdtemp)
            temp_file = f"{temp_dir}/{self.repository_manifest.filename}"

            result = await self.vais.async_save_file(temp_file, filecontent)
            ...
            def cleanup_temp_dir():
                ...
                if os.path.exists(temp_dir):
                    self.logger.debug("%s Cleaning up %s",
                                      self.string, temp_dir)
                    shutil.rmtree(temp_dir)

            if result:
                self.logger.info("%s Download of %s completed",
                                 self.string, content.name)
                await self.vais.hass.async_add_executor_job(cleanup_temp_dir)
                return
    ...
    async def download_content(self) -> None:
        ...
        if self.vais.configuration.experimental:
            if (
                ...
                self.logger.info(
                    "%s Trying experimental download", self.string)
                try:
        ...
        if self.repository_manifest.filename:
            self.logger.debug("%s %s", self.string,
                              self.repository_manifest.filename)
        if not contents:
            raise VaisException("No content to download")
    ...
    async def download_repository_zip(self):
        ...
        if not ref:
            raise VaisException("Missing required elements.")

        url = f"{BASE_API_URL}/repos/{self.data.full_name}/zipball/{ref}"

        filecontent = await self.vais.async_download_file(
            url,
            headers={
                "Authorization": f"token {self.vais.configuration.token}",
                "User-Agent": f"VAIS/{self.vais.version}",
            },
        )
        if filecontent is None:
            raise VaisException(f"[{self}] Failed to download zipball")

        temp_dir = await self.vais.hass.async_add_executor_job(tempfile.mkdtemp)
        temp_file = f"{temp_dir}/{self.repository_manifest.filename}"
        result = await self.vais.async_save_file(temp_file, filecontent)
        if not result:
            raise VaisException("Could not save ZIP file")
        ...
        await self.vais.hass.async_add_executor_job(cleanup_temp_dir)
        self.logger.info("%s Content was extracted to %s",
                         self.string, self.content.path.local)

    async def async_get_vais_json(self, ref: str = None) -> dict[str, Any] | None:
        """Get the content of the vais.json file."""
        try:
            response = await self.vais.async_github_api_method(
                method=self.vais.githubapi.repos.contents.get,
                raise_exception=False,
                repository=self.data.full_name,
                path=RepositoryFile.VAIS_JSON,
    ...
    async def async_get_info_file_contents(self) -> str:
        ...
        def _info_file_variants() -> tuple[str, ...]:
            name: str = (
                "readme"
                if self.repository_manifest.render_readme or self.vais.configuration.experimental
                else "info"
            )
            return (
                f"{name.upper()}.md",
        ...
        info_files = [filename for filename in _info_file_variants()
                      if filename in self.treefiles]
    ...
    def remove(self) -> None:
        ...
        if self.vais.repositories.is_registered(repository_id=str(self.data.id)):
            self.vais.repositories.unregister(self)

    async def uninstall(self) -> None:
        """Run uninstall tasks."""
        self.logger.info("%s Removing", self.string)
        if not await self.remove_local_directory():
            raise VaisException("Could not uninstall")
        ...
        await async_remove_store(self.vais.hass, f"vais/{self.data.id}.vais")
    ...
    async def remove_local_directory(self) -> None:
        ...
        try:
            if self.data.category == "python_script":
                ...
                path = (
                    f"{self.vais.core.config_path}/"
                    f"{self.vais.configuration.theme_path}/"
            ...
            if os.path.exists(local_path):
                if not is_safe(self.vais, local_path):
                    self.logger.error(
        ...
        # lgtm [py/catch-base-exception] pylint: disable=broad-except
            ...
            self.logger.debug("%s Removing %s failed with %s",
                              self.string, local_path, exception)
            return False
        return True
    ...
    async def async_post_registration(self) -> None:
        ...
        if not self.vais.system.action:
            return
        await self.vais.validation.async_run_repository_checks(self)
    ...
    async def async_install(self) -> None:
        ...
        self.vais.async_dispatch(
            VaisDispatchEvent.REPOSITORY_DOWNLOAD_PROGRESS,
    ...
    async def async_install_repository(self) -> None:
        ...
        if self.content.path.local is None:
            raise VaisException("repository.content.path.local is None")
        self.validate.errors.clear()
        ...
        if self.data.installed and self.data.category == "netdaemon":
            persistent_directory = BackupNetDaemon(
                vais=self.vais, repository=self)
            await self.vais.hass.async_add_executor_job(persistent_directory.create)
            ...
            if os.path.exists(
                ...
                persistent_directory = Backup(
                    ...
                    backup_path=tempfile.gettempdir() + "/vais_persistent_directory/",
                )
                await self.vais.hass.async_add_executor_job(persistent_directory.create)

        if self.data.installed and not self.content.single:
            backup = Backup(vais=self.vais, local_path=self.content.path.local)
            await self.vais.hass.async_add_executor_job(backup.create)

        self.vais.log.debug("%s Local path is set to %s",
                            self.string, self.content.path.local)
        self.vais.log.debug("%s Remote path is set to %s",
                            self.string, self.content.path.remote)
        ...
        if self.validate.errors:
            ...
            if self.data.installed and not self.content.single:
                await self.vais.hass.async_add_executor_job(backup.restore)
                await self.vais.hass.async_add_executor_job(backup.cleanup)
            raise VaisException("Could not download, see log for details")
        ...
        if persistent_directory is not None:
            await self.vais.hass.async_add_executor_job(persistent_directory.restore)
            await self.vais.hass.async_add_executor_job(persistent_directory.cleanup)
    ...
    async def async_get_legacy_repository_object(
        ...
        try:
            repository = await self.vais.github.get_repo(self.data.full_name, etag)
            return repository, self.vais.github.client.last_response.etag
        except AIOGitHubAPINotModifiedException as exception:
            raise VaisNotModifiedException(exception) from exception
        except (ValueError, AIOGitHubAPIException, Exception) as exception:
            raise VaisException(exception) from exception
    ...
    async def get_tree(self, ref: str):
        ...
        if self.repository_object is None:
            raise VaisException("No repository_object")
        try:
    ...
    async def get_releases(self, prerelease=False, returnlimit=5) -> list[GitHubReleaseModel]:
        ...
        response = await self.vais.async_github_api_method(
            method=self.vais.githubapi.repos.releases.list,
            repository=self.data.full_name,
        )
        releases = []
    ...
    async def common_update_data(self, ignore_issues: bool = False, force: bool = False) -> None:
        ...
        try:
            ...
            if self.data.full_name.lower() != repository_object.full_name.lower():
                ...
                raise VaisRepositoryExistException
            self.data.update_data(repository_object.attributes)
            self.data.etag_repository = etag
        except VaisNotModifiedException:
            return
        except VaisRepositoryExistException:
            raise VaisRepositoryExistException from None
        except (AIOGitHubAPIException, VaisException) as exception:
        ...
        if self.data.archived and not ignore_issues:
            ...
            if self.data.full_name not in self.vais.common.archived_repositories:
                self.vais.common.archived_repositories.append(
                    self.data.full_name)
            raise VaisRepositoryArchivedException(
                f"{self} Repository is archived.")

        # Make sure the repository is not in the blacklist.
        if self.vais.repositories.is_removed(self.data.full_name):
            removed = self.vais.repositories.removed_repository(
                self.data.full_name)
        ...
        try:
            releases = await self.get_releases(
                ...
                returnlimit=self.vais.configuration.release_limit,
            )
            if releases:
                self.data.releases = True
                self.releases.objects = releases
                self.data.published_tags = [
                    x.tag_name for x in self.releases.objects]
        ...
        self.vais.log.debug(
            "%s Running checks against %s", self.string, self.ref.replace(
                "tags/", "")
        )
        ...
        try:
            ...
            if not self.tree:
                raise VaisException("No files in tree")
            self.treefiles = []
            for treefile in self.tree:
                self.treefiles.append(treefile.full_path)
        except (AIOGitHubAPIException, VaisException) as exception:
            if not self.vais.status.startup and not ignore_issues:
                self.logger.error("%s %s", self.string, exception)
            if not ignore_issues:
                raise VaisException(exception) from None
    ...
    def gather_files_to_download(self) -> list[FileInformation]:
        ...
        for path in tree:
            ...
            if path.full_path.startswith(self.content.path.remote):
                files.append(FileInformation(path.download_url,
    ...
    async def dowload_repository_content(self, content: FileInformation) -> None:
        ...
        try:
            ...
            filecontent = await self.vais.async_download_file(content.download_url)
            ...
            result = await self.vais.async_save_file(local_file_path, filecontent)
            if result:
                self.logger.info("%s Download of %s completed",
                                 self.string, content.name)
                return
        ...
        # lgtm [py/catch-base-exception] pylint: disable=broad-except
            ...
            self.validate.errors.append(
                f"Download was not completed [{exception}]")

    async def async_remove_entity_device(self) -> None:
        """Remove the entity device."""
        if (
            self.vais.configuration == ConfigurationType.YAML
            or not self.vais.configuration.experimental
        ):
            return

        device_registry: dr.DeviceRegistry = dr.async_get(hass=self.vais.hass)
