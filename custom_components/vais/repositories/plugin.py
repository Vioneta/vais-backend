...
class VaisPluginRepository(VaisRepository):
    """Plugins in VAIS."""
    ...
    def __init__(self, vais: VaisBase, full_name: str):
        ...
        self.data.category = VaisCategory.PLUGIN
        self.content.path.local = self.localpath

    @property
    def localpath(self):
        """Return localpath."""
        return f"{self.vais.core.config_path}/www/community/{self.data.full_name.split('/')[-1]}"
    ...
    def update_filenames(self) -> None:
        ...
        possible_locations = (
            ("",) if self.repository_manifest.content_in_root else (
                "release", "dist", "")
        )
