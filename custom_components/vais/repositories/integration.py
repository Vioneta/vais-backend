"""Class for integrations in VAIS."""
...
from ..enums import VaisCategory, VaisDispatchEvent, VaisGitHubRepo, RepositoryFile
from ..exceptions import AddonRepositoryException, VaisException
...
class VaisIntegrationRepository(VaisRepository):
    """Integrations in VAIS."""
    ...
    def __init__(self, vais: VaisBase, full_name: str):
        ...
        self.data.category = VaisCategory.INTEGRATION
    ...
    def localpath(self):
        """Return localpath."""
        return f"{self.vais.core.config_path}/custom_components/{self.data.domain}"

    async def async_post_installation(self):
        """Run post installation steps."""
        if self.data.config_flow:
            if self.data.full_name != VaisGitHubRepo.INTEGRATION:
    ...
    async def reload_custom_components(self):
        ...
        del self.vais.hass.data["custom_components"]
        await async_get_custom_components(self.vais.hass)
