...
def get_store_key(key):
    ...
    return key if "/" in key else f"vais.{key}"


def _get_store_for_key(hass, key, encoder):
    """Create a Store object for the key."""
    return VAISStore(hass, VERSION_STORAGE, get_store_key(key), encoder=encoder, atomic_writes=True)
...
async def async_save_to_store(hass, key, data):
    ...
    if current is None or current != data:
        ...
        return
    _LOGGER.debug(
        "<VAISStore async_save_to_store> Did not store data for '%s'. Content did not change",
        get_store_key(key),
    )
