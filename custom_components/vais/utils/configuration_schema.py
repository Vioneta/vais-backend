...
def vais_base_config_schema(config: dict = {}) -> dict:
    """Return a shcema configuration dict for VAIS."""
    if not config:
        config = {
...
def vais_config_option_schema(options: dict = {}) -> dict:
    """Return a shcema for VAIS configuration options."""
    if not options:
        options = {
            APPDAEMON: False,
            COUNTRY: "ALL",
            ...
            NETDAEMON: False,
            RELEASE_LIMIT: 5,
            SIDEPANEL_ICON: "vais:vais",
            SIDEPANEL_TITLE: "VAIS",
            FRONTEND_REPO: "",
            FRONTEND_REPO_URL: "",
        }
    return {
...
def vais_config_combined() -> dict:
    """Combine the configuration options."""
    base = vais_base_config_schema()
    options = vais_config_option_schema()
