...
from custom_components.vais.const import VERSION_STORAGE
from custom_components.vais.exceptions import VaisException
from custom_components.vais.utils.store import (
...
async def test_store_load(hass: HomeAssistant) -> None:
    ...
    with patch(
        "custom_components.vais.utils.store.json_util.load_json",
    ...
    with patch("custom_components.vais.utils.store.json_util.load_json", return_value={}):
        assert store.load() is None

    with pytest.raises(VaisException):
        with patch(
            "custom_components.vais.utils.store.json_util.load_json", side_effect=OSError("No file")
        ):
            assert store.load() == {"test": "test"}
...
async def test_store_remove(hass: HomeAssistant) -> None:
    ...
    with patch(
        "custom_components.vais.utils.store.VAISStore.async_remove", return_value=AsyncMock()
...
async def test_store_store(hass: HomeAssistant, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch(
        "custom_components.vais.utils.store.VAISStore.async_save", return_value=AsyncMock()
        ...
        assert (
            "<VAISStore async_save_to_store> Did not store data for 'vais.test'. Content did not change"
            in caplog.text
        )
